        ...
        <div className="config-tier-info">
          ...
          {subscriptionTier === 'none' && (
            <div className="config-subscription-required">
              ...
              <button 
                className="config-upgrade-button subscription-button" 
                onClick={() => {
                  console.log('Subscribe button clicked from HuggingFaceConfig');
                  // Direct access to parent's upgradeSubscription function
                  if (window.parent) {
                    window.parent.postMessage({ type: 'SUBSCRIBE_NOW' }, '*');
                  }
                  // As a fallback, also try to trigger a custom event
                  const event = new CustomEvent('subscriptionUpgradeRequested');
          ...
          {subscriptionTier === 'basic' && (
            <div className="config-tier-limit">
              ...
              {selectedModels.length >= 3 && (
                <button 
                  className="config-upgrade-button subscription-button"
                  onClick={() => window.parent.postMessage({ type: 'UPGRADE_SUBSCRIPTION', tier: 'complete' }, '*')}
                >
                  Upgrade to Complete
                </button>
              )}
