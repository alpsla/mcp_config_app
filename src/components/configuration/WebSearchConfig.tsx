...
const WebSearchConfig: React.FC<WebSearchConfigProps> = ({ config = {}, updateConfig }) => {
  // Initialize state with values from props or defaults
  const [resultsCount, setResultsCount] = useState<number>(config.resultsCount || 3);
  const [safeSearch, setSafeSearch] = useState<boolean>(config.safeSearch !== false); // Default to true
  ...
  const handleResultsCountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
