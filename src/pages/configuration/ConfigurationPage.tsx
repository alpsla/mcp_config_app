...
interface ConfigurationsState {
  webSearch: ServerConfig;
  fileSystem: ServerConfig;
  huggingFace: ServerConfig;
}
...
const ConfigurationPage: React.FC<ConfigurationPageProps> = ({ history, onSaveConfiguration }) => {
  ...
  // Configuration name
  const [configName, setConfigName] = useState<string>("My MCP Configuration");
  ...
  const updateServerConfig = (serverType: keyof ConfigurationsState, newConfig: Record<string, any>) => {
    setConfigurations(prevConfig => ({
      ...prevConfig,
      [serverType]: {
        ...prevConfig[serverType],
  ...
  const formatWebSearchArgs = (config: Record<string, any>): string[] => {
    const args: string[] = ["--results-count", String(config.resultsCount || "3")];
  ...
  const formatFileSystemArgs = (config: Record<string, any>): string[] => {
    const args: string[] = [];
  ...
  const formatHuggingFaceArgs = (config: Record<string, any>): string[] => {
    const args: string[] = [];
